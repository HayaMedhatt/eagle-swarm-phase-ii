...
class MissionDemo(Node):
    def __init__(self) -> None:
        ...
        self.declare_parameter("land_delay_sec", 0.0)
    ...
    def tick(self) -> None:
        if self.stage == "WAIT_ACTIVE":
            ...
            if not self.winner_id:
                return
            # Land the target winner first. Keeping the other aircraft in their
            # sectors makes the sequence visually clear and prevents their RTB
            # motion from distracting from target touchdown.
            self.mission_pub.publish(String(data=f"land:{self.winner_id}"))
            self.land_sent = True
            self.set_stage("WINNER_LANDING")
            self.get_logger().warn(
                f"{self.winner_id} DIRECT TARGET LAND commanded; "
                "non-winners holding sectors until touchdown"
            )

        elif self.stage == "WINNER_LANDING":
            winner = self.heartbeats.get(self.winner_id or "")
            if winner is not None and winner.state == "LANDED":
                for robot_id in sorted(self.required):
                    if robot_id != self.winner_id:
                        self.mission_pub.publish(
                            String(data=f"return_home:{robot_id}")
                        )
                self.set_stage("RETURNING")
                self.get_logger().warn(
                    f"{self.winner_id} landed on target; "
                    "non-winning aerial units returning home"
                )
            ...
            if landed:
                ...
                self.get_logger().warn(
                    ...
                    "allocation, winner target landing and non-winner RTB"
                )
