...
printf '\nWaiting for Digital Twin dashboard...\n'
if ! wait_for_dashboard 35; then
  tail -n 120 "$LOG_DIR/real_swarm.log" >&2 || true
  "$WS_DIR/scripts/stop_integrated_demo.sh" || true
  fail "Digital Twin dashboard did not become ready on http://127.0.0.1:8080"
fi
printf '  dashboard ready at http://127.0.0.1:8080\n'

if [[ "$OPEN_DASHBOARD" == "1" ]]; then
  if command -v xdg-open >/dev/null 2>&1; then
    xdg-open http://127.0.0.1:8080 >/dev/null 2>&1 || true
  else
    printf '  xdg-open unavailable; open the dashboard URL manually.\n'
  fi
fi
